export const openApiDocument = {
  ...
  paths: {
    '/auth/register': { post: { summary: 'Register a user', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { username: { type: 'string', minLength: 3, maxLength: 50 }, password: { type: 'string', minLength: 8, maxLength: 128 } }, required: ['username', 'password'] } } } }, responses: { '201': { description: 'User registered' }, '409': { description: 'Username already exists' } } } },
    '/auth/login': { post: { summary: 'Create a JWT', requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', properties: { username: { type: 'string' }, password: { type: 'string' } }, required: ['username', 'password'] } } } }, responses: { '200': { description: 'Token generated' }, '401': { description: 'Invalid credentials' } } } },
