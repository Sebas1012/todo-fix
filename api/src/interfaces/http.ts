...
export const taskRouter = (repository: TaskRepository) => {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    ...
    const result = await list.execute({ completed: query.completed === undefined ? undefined : query.completed === 'true', page: query.page, limit: query.limit });
    res.json({ data: result.items.map(presentTask), pagination: result.pagination });
  }));

  router.post('/', asyncHandler(async (req, res) => {
    const task = await create.execute(createSchema.parse(req.body));
    res.status(201).json({ data: presentTask(task) });
  }));

  router.get('/:id', asyncHandler(async (req, res) => res.json({ data: presentTask(await get.execute(idSchema.parse(req.params.id))) })));
  router.patch('/:id', asyncHandler(async (req, res) => res.json({ data: presentTask(await update.execute(idSchema.parse(req.params.id), updateSchema.parse(req.body))) })));
  router.delete('/:id', asyncHandler(async (req, res) => { await remove.execute(idSchema.parse(req.params.id)); res.status(204).send(); }));
